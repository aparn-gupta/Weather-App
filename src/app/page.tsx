...
const handleStatus = (errStatus: any) => {
 ...
 <div className=" grey-transparent w-11/12 lg:w-1/2 text-white p-8 rounded-md  shadow-2xl">  
...
<div className=  {` w-screen flex justify-center  ${searchResultsShowing ? "h-48 md:h-52 lg:h-24 bg-white absolute top-0 left-0 py-3 px-2 lg:px-8 " : " bg-white lg:flex-col lg:items-center h-screen mt-20 lg:mt-0" }`}      > 
  <div  className= {`${searchResultsShowing ? "  w-3/5 lg:w-screen lg:flex lg:justify-start"  : "w-5/6 lg:w-auto"}`} >
 

  <div onClick={() => {setSearchResultsShowing(false)}}  className={`font-bold w-full flex  ${searchResultsShowing ? " text-2xl md:text-4xl lg:text-3xl mt-3 justify-center lg:justify-start " : "text-2xl lg:text-8xl  justify-center" }`}> <p className="text-blue-600"> L </p>  <p className="text-red-500"> i </p> <p className="text-yellow-500"> v </p> <p className="text-blue-600"> e </p> <p className={`text-green-600 ml-1.5   ${searchResultsShowing ? "md:ml-1.5" : " md:ml-4"}`}> W </p> <p className="text-red-500"> e </p> <p className="text-yellow-500"> a </p> <p className="text-blue-600"> t </p> <p className="text-red-500"> h </p > <p className="text-green-600"> e </p> <p className="text-yellow-500"> r </p> </div>

  <div className={`rounded-3xl  ${searchingByCoordinates ? "border-0"  :"border border-zinc-300 hover:shadow-md"  }   ${searchResultsShowing ? "w-full  my-3 lg:my-0 ml-1 "  : "w-full  mt-6" }`}   >
  <input className=   {`bg-white w-full rounded-3xl px-3 pt-1.5 lg:pt-4 outline-0  hover:border-0 ${searchingByCoordinates ? "hidden" : "block" }  `}  type="text" value={userInput} onChange={handleChange} name = "userInputPlace" id = "userInput"  placeholder="Type Locality e.g. Brookefields"/>
  <div className=  {`justify-between ${searchingByCoordinates ? "flex" : "hidden" } `} > 
  <input className= {` font-bold rounded-3xl p-2.5 outline-0 hover:shadow-md border border-zinc-300 ${searchResultsShowing ? " w-28 md:w-52 mt-3" : "w-5/12"} `}  placeholder="Latitude"  type="text" name= "latitude" value={latitude} onChange={(e) => setLatitude(e.target.value)}/>
  <input className= {`font-bold rounded-3xl p-2.5 outline-0 hover:shadow-md border border-zinc-300 ${searchResultsShowing ? " w-28  md:w-52 mt-3" : "w-5/12"}`}  placeholder="Longitude"  type="text" name= "longitude" value={longitude} onChange={(e) => setLongitude(e.target.value)}/>
  ...
  <div className=  {` lg:max-h-12 flex w-full text-sm lg:text-base  ${searchResultsShowing?  "mt-2 md:mt-0 lg:justify-end lg:mr-20" : "mt-8 justify-center" }`} > 
